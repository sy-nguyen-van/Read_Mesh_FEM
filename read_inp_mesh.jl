...
Returns:
    Dict with:
      ...
      "elem_sets"   => Dict("SETNAME" => [elem_ids])
"""
function read_inp_mesh(file_path::String)
    ...
    elem_sets = Dict{String,Vector{Int}}()
    ...
    open(file_path, "r") do io
        for line in eachline(io)
            ...
            elseif startswith(uppercase(line), "*ELSET")
                ...
                m = match(r"(?i)elset\s*=\s*([A-Za-z0-9_]+)", line)
                setname = m === nothing ? "UNKNOWN" : m.captures[1]
                m2 = match(r"(?i)instance\s*=\s*([A-Za-z0-9_\-]+)", line)
                instname = m2 === nothing ? "" : m2.captures[1]
                current_set_key = instname == "" ? setname : "$(setname)"
                elem_sets[current_set_key] = Int[]
            ...
            elseif current_section == :elset && current_set_key !== nothing
                ...
                for p in parts
                    p = strip(p)
                    if !isempty(p)
                        push!(elem_sets[current_set_key], parse(Int, p))
                    end
                end
            end
        end
